...
class Location extends Base {
    ...
    static joinAll (locations, resolver) {
        ...
        for (let loc of locations) {
            ...
            if (loc) {
                ...
                ret += Location.format(file, loc.line, loc.column);
            }
            else {
                ret += '??';
            }
    ...
    static split (src, resolver) {
        let locations = src.split(':');

        for (let i = locations.length; i-- > 0; ) {
            locations[i] = Location.from(locations[i], resolver);
        }
    ...
    generate (values) {
        ...
        for (let v of values) {
            ret.push(new Location(this).advanceColumn(offset));
