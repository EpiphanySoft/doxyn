...
class Chunk extends Base {
    static fieldOf (chunk, index) {
        let begin = 0;
        let end = chunk.indexOf(this.delimiter);
        let i, s;

        for (i = 0; i < index && end > -1; ++i) {
            begin = end + 1;
            end = chunk.indexOf(this.separator, begin);
        }
    ...
    constructor (fileIndex, line, column, length) {
        super();

        if (fileIndex.isSrcChunk) {
            this.fileIndex = fileIndex.fileIndex;
            this.line = fileIndex.line;
            this.column = fileIndex.column;
            this.begin = fileIndex.begin;
            this.length = fileIndex.length;
            this.owner = fileIndex.owner;
        }
        else {
    ...
    get file () {
        ...
        return files && files[this.fileIndex] || null;
    }
