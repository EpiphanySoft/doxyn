...
class Node extends Entity {
    static define (members) {
        let tagName = members.tagName;

        if (tagName) {
            delete members.tagName;

            Object.defineProperty(this.prototype, '_tagName', {
                value: tagName
            });
        }

        let multiLine = members.multiLineAttributes;

        if (multiLine) {
            delete members.multiLineAttributes;
            this.multiLine = Object.create(Object.getPrototypeOf(this).multiLine);

            if (Array.isArray(multiLine)) {
                for (let name of multiLine) {
                    this.multiLine[name] = true;
                }
            }
            else {
                Object.assign(this.multiLine, multiLine);
            }
        }
