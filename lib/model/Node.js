...
class Node extends Entity {
    ...
    appendAttribute (name, value, at) {
        ...
        if (!attr.composite && !attr.multiline) {
            throw new Error(`Cannot append values to normal attribute "${name}"`);
        }
        ...
        let currentSrc = this.src[name];
        let currentLoc = this.loc[name];

        if (current) {
            if (attr.composite) {
                current.push(...values);

                // We always have a value for src and currentSrc (if there is a
                // current value). This is because we store "??" to represent the
                // null values.
                ...
                if (src) {
                    if (currentLoc) {
                        if (!currentSrc) {
                            currentSrc = currentLoc.toString();
                        }
                        currentLoc = null;
                    }
                    currentSrc += Location.delimiter;
                    ...
                    if (currentLoc) {
                        currentLoc.concat(loc);
                        currentSrc = null;
                    }
                    else {
                        currentSrc += Location.delimiter;
                        currentSrc += loc.toString();
                    }
                }
            }
        }
    ...
    normalizeAttribute (name, value, at) {
        ...
        if (at) {
            if (at.isSourceMap) {
                ...
                if (at.files !== doc.files) {
                    at.attach(doc.files, f => doc.getFile(f));
                }
