...
class Node extends Entity {
    ...
    getAttributeSrc (name) {
        ...
        if (!src) {
            ...
            if (src) {
                src = src.isInstance ? src.toString() : Location.join(src);
                this.src[name] = src;
            }
        }
    ...
    normalizeAttribute (name, value, at) {
        ...
        if (at) {
            ...
            else if (at.isLocation || Array.isArray(at)) {
                loc = at;
            }
        ...
        if (attr.multiline) {
            ...
            if (loc) {
                ...
                let doc = this.ownerDocument;
                src = Location.joinAll(loc, f => doc.getFileIndex(f));
            }
            else {
                if (!at) {
                    loc = [];
