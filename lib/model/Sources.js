...
class Sources {
    ...
    insert (offset, str) {
        ...
        if (n && index >= 0) {
            if (text) {
                me.text = text.substr(0, offset) + str + text.substr(offset);
            }

            chunk = chunks[index];
            chunk.length += n;
            
            for (prev = chunk; index < chunks.length; ++index) {
                chunk.begin = prev.end;
            }
    ...
    replace (offset, length, str) {
        return this.erase(offset, length).insert(offset, str);
    }
