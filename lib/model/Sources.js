...
class Chunk extends Location {
    ...
    scan (offset, cursor) {
        let offsets = this.splitOffset(offset);

        if (!offsets) {
            return false;
        }

        cursor.change(this);  // copy our location into the cursor
        cursor.advance(offsets[0]);
        cursor.advanceColumn(offsets[1]);
...
class Sources extends Base {
    ...
    replace (offset, length, str) {
        ...
        if (count > 1) {
            let [line, col] = chunk.splitOffset(offset);
    ...
    splitChunkByLine (chunk, numLines) {
        ...
        let next = this.splitChunkAt(length);
    ...
    splitChunkIntraLine (chunk, columnOffset) {
        if (chunk.lineCount() > 1) {
            throw new Error('Cannot split a multi-line chunk as intra-line');
        }

        let next = this.splitChunkAt(columnOffset);
